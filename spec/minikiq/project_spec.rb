  ...
  context 'validating a project' do
    ...
    describe 'self.check_amount_dollar_sign(amount)' do
      ...
      it 'returns true if amount has no dollar sign' do
        expect(Minikiq::CLI::Project.check_amount_dollar_sign(good_amount)).to eq true
      end

      it 'returns false if amount has dollar sign' do
        expect(Minikiq::CLI::Project.check_amount_dollar_sign(bad_amount)).to eq false
      end
    end
    ...
    describe 'self.check_input_length(input)' do
      ...
      it 'returns true if correct number of args' do
        expect(Minikiq::CLI::Project.check_input_length(good_input)).to eq true
      end

      it 'returns false if incorrect number of args' do
        expect(Minikiq::CLI::Project.check_input_length(short_input)).to eq false
        expect(Minikiq::CLI::Project.check_input_length(long_input)).to eq false
      end
    end
    ...
    describe 'self.check_name_characters(name)' do
      ...
      it 'returns true if name has correct chacters' do
        expect(Minikiq::CLI::Project.check_name_characters(good_name)).to eq true
      end

      it 'returns false if name does not have correct chacters' do
        expect(Minikiq::CLI::Project.check_name_characters(bad_name)).to eq false
      end
    end
    ...
    describe 'self.check_name_length(name)' do
      ...
      it 'returns true if name is between 4 and 20 characters long' do
        expect(Minikiq::CLI::Project.check_name_length(good_name)).to eq true
      end

      it 'returns false if name is not between 4 and 20 characters long' do
        expect(Minikiq::CLI::Project.check_name_length(short_name)).to eq false
        expect(Minikiq::CLI::Project.check_name_length(long_name)).to eq false
      end
    end
  end
end
